...
- **Корректировки ставок** — мобильные, десктопные, демографические, региональные, видео и ретаргетинг
- **Ретаргетинг** — списки ретаргетинга и аудиторные таргетинги
- **Смарт-таргеты** — фильтры по фидам
- **Динамические текстовые таргеты** — автотаргетинг для динамических объявлений
...
- **Лид-формы** — управление формами генерации лидов
- **Турбостраницы** — быстрые мобильные посадочные страницы
...
Затем добавьте в `~/.openclaw/openclaw.json`:
...
### Вариант 2: Через uvx (Без установки)
...
Добавьте переменные окружения в конфигурацию OpenClaw:
...
Или используйте OAuth-авторизацию для автоматического обновления токена:
...
```bash
openclaw mcp list
# Должно показать: yandex-direct

openclaw mcp show yandex-direct --json
# Должно показать все 170 инструментов
```

Подробности см. в [Руководстве по быстрому старту для OpenClaw](docs/openclaw-quickstart.md).
...
| Инструмент                   | Описание                                                                                              |
| ---------------------------- | ----------------------------------------------------------------------------------------------------- |
| `direct_get_campaigns`       | Получить список кампаний со статусом, стратегией, бюджетом, кошельком, уведомлениями и часовым поясом |
...
| `direct_suspend_adgroups`   | Приостановить группы объявлений          |
| `direct_resume_adgroups`    | Возобновить группы объявлений            |
| `direct_archive_adgroups`   | Архивировать группы объявлений           |
| `direct_unarchive_adgroups` | Разархивировать группы объявлений        |
...
| Инструмент                    | Описание                                                                                    |
| ----------------------------- | ------------------------------------------------------------------------------------------- |
| `direct_get_bid_modifiers`    | Получить корректировки (мобильные, десктоп, демография, регионы, видео, ретаргетинг)        |
| `direct_add_bid_modifier`     | Добавить корректировку ставки (мобильные, десктоп, демография, регионы, видео, ретаргетинг) |
...
#### Динамические текстовые таргеты (6)

| Инструмент                               | Описание                                                |
| ---------------------------------------- | ------------------------------------------------------- |
| `direct_get_dynamic_text_ad_targets`     | Получить фильтры динамических текстовых объявлений      |
| `direct_add_dynamic_text_ad_target`      | Добавить автотаргетинг в группу динамических объявлений |
| `direct_update_dynamic_text_ad_target`   | Обновить настройки динамического таргетинга             |
| `direct_suspend_dynamic_text_ad_targets` | Приостановить динамические таргеты                      |
| `direct_resume_dynamic_text_ad_targets`  | Возобновить динамические таргеты                        |
| `direct_delete_dynamic_text_ad_targets`  | Удалить динамические таргеты                            |
...
| `direct_delete_advideos`           | Удалить видео                                          |
...
| Инструмент                   | Описание                                            |
| ---------------------------- | --------------------------------------------------- |
| `direct_get_lead_forms`      | Получить лид-формы с фильтрацией по кампании/группе |
| `direct_add_lead_form`       | Создать новую лид-форму с вопросами                 |
| `direct_update_lead_form`    | Обновить настройки лид-формы                        |
| `direct_delete_lead_forms`   | Удалить лид-формы                                   |
| `direct_get_lead_form_leads` | Получить лиды из лид-форм                           |

#### Турбостраницы (5)

| Инструмент                        | Описание                                               |
| --------------------------------- | ------------------------------------------------------ |
| `direct_get_turbo_pages`          | Получить список турбостраниц со статусом и настройками |
| `direct_add_turbo_page`           | Создать новую турбостраницу из URL сайта               |
| `direct_update_turbo_page`        | Обновить название и URL турбостраницы                  |
| `direct_delete_turbo_pages`       | Удалить турбостраницы                                  |
| `direct_get_turbo_page_templates` | Получить доступные шаблоны турбостраниц                |
